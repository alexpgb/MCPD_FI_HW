...
#    search_text = 'data science python'
    ...
    while True:
        ...
        for vacancy_item in vacancy_list_as_tag:
            num += 1
            vacancy_result['site'].append(site.split('/')[2])
            vacancy_result['title'].append(
                vacancy_item.find(hh_searh_vacancy_name['el'], hh_searh_vacancy_name['attrs']).
                    getText().replace('\xa0', ' '))
            ...
            if salary is not None:
                ...
                if salary.split()[0] == 'до':
                    vacancy_result['salary_max'][num] = int(salary.split()[1])
                if salary.split()[0] == 'от':
                    vacancy_result['salary_min'][num] = int(salary.split()[1])
                if salary.split()[1] == '–':
                    vacancy_result['salary_min'][num] = int(salary.split()[0])
                    vacancy_result['salary_max'][num] = int(salary.split()[2])
...
with MongoClient('localhost', 27017) as client:
    db = client['vacancy']
    col_hh = db.content
    fields = list(vacancy_result.keys())
    item_added = 0
    for i in range(len(vacancy_result[list(vacancy_result.keys())[0]])):
        doc = {fields: vacancy_result[fields][i] for j, fields in enumerate(fields, 0)}
        doc_list = list(col_hh.find({'url': vacancy_result['url'][i]}, {'_id': 1}))
        if len(doc_list) == 0:
